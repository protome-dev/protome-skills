...
const SELECTION_STATE_ELEMENT_ID = 'proto-me-selection-state'
...
const ANNOTATION_TOOL_ID = 'proto-me-annotation'
...
const annotationToolIcon = (
  <div
    className="proto-me-annotation-tool-icon"
...
const REMOTE_SYNCED_RECORD_TYPES = new Set(['page', 'shape', 'asset', 'binding'])
const REMOTE_REMOVAL_ORDER = new Map([
  ['binding', 0],
  ['shape', 1],
  ['asset', 2],
  ['page', 3]
...
function isRemoteSyncedRecord(record) {
  return REMOTE_SYNCED_RECORD_TYPES.has(record?.typeName)
}
...
function applyRemoteCanvasSnapshot(editor, snapshot, { preserveLocalChanges = false } = {}) {
  ...
  const remoteStore = sanitized.snapshot.store
  const remoteIds = new Set(Object.keys(remoteStore))
  const currentStore = editor.store.getStoreSnapshot().store
  const recordIdsToRemove = preserveLocalChanges
    ? []
    : Object.values(currentStore)
        .filter((record) => isRemoteSyncedRecord(record) && !remoteIds.has(record.id))
        .sort((left, right) => {
          return (REMOTE_REMOVAL_ORDER.get(left.typeName) ?? 99) - (REMOTE_REMOVAL_ORDER.get(right.typeName) ?? 99)
        })
  ...
  if (recordsToPut.length === 0 && recordIdsToRemove.length === 0) {
  ...
  editor.store.mergeRemoteChanges(() => {
    if (recordIdsToRemove.length > 0) {
      ...
      changedRecords += recordIdsToRemove.length
    }
...
function getAiImageHolderMeta() {
  return {
    ...
    protoMeAiImageHolderVersion: 1
  }
}

function isAiImageHolderShape(shape) {
  return shape?.type === 'frame' && shape.meta?.protoMeAiImageHolder === true
}

function isAiImageAspectLocked(shape) {
  return isAiImageHolderShape(shape) && shape.meta?.protoMeAiAspectLocked === true
}
...
function getAiImageAspectRatio(shape) {
  const metaRatio = Number(shape?.meta?.protoMeAiAspectRatio)
...
function pinAnnotationArrowLabelPosition(editor, arrowId, attempt = 0) {
  editor.timers.setTimeout(() => {
    const shape = editor.getShape(arrowId)
    if (!shape || shape.meta?.protoMeAnnotationArrow !== true) return
...
class ProtoMeAnnotationTool extends StateNode {
  static id = ANNOTATION_TOOL_ID
  static initial = 'idle'

  static children() {
    return [ProtoMeAnnotationIdle, ProtoMeAnnotationPointing]
  }
...
class ProtoMeAnnotationIdle extends StateNode {
  static id = 'idle'
...
class ProtoMeAnnotationPointing extends StateNode {
  static id = 'pointing'
...
class ProtoMeFrameShapeUtil extends FrameShapeUtil {
...
const protoMeShapeUtils = [ProtoMeFrameShapeUtil]

const protoMeUiOverrides = {
  translations: {
    en: {
      'tool.ai-image': AI_IMAGE_HOLDER_LABEL,
      'tool.proto-me-annotation': ANNOTATION_TOOL_LABEL
    },
    'zh-cn': {
      'tool.ai-image': AI_IMAGE_HOLDER_LABEL,
      'tool.proto-me-annotation': ANNOTATION_TOOL_LABEL
  ...
  tools(editor, tools) {
    return {
      ...
      [AI_IMAGE_TOOL_ID]: {
        ...
        meta: {
          protoMeTool: 'ai-image-holder'
        }
      },
      [ANNOTATION_TOOL_ID]: {
        id: ANNOTATION_TOOL_ID,
        label: 'tool.proto-me-annotation',
...
const protoMeComponents = {
  Toolbar: ProtoMeToolbar,
  StylePanel: ProtoMeStylePanel
}

function ProtoMeStylePanel(props) {
  return (
    <DefaultStylePanel {...props}>
      <DefaultStylePanelContent />
      <ProtoMeAiImageStyleControls />
    </DefaultStylePanel>
  )
}

function ProtoMeAiImageStyleControls() {
    ...
    editor.updateShapes([
      {
        ...
        meta: {
          ...selectedAiImageShape.meta,
          protoMeAiAspectLocked: nextIsLocked,
          protoMeAiAspectRatio: currentRatio
        }
      }
    ])
  }
  ...
  return (
    <div className="proto-me-ai-image-style-panel" aria-label="AI 图片尺寸设置">
      <section className="proto-me-ai-style-section">
        <div className="proto-me-ai-style-heading">
          <span>尺寸</span>
        </div>
        <div className="proto-me-ai-size-row">
          <label className="proto-me-ai-size-field">
            <span>W</span>
            <input
              aria-label="AI 图片宽度"
              inputMode="numeric"
          ...
          <button
            ...
            className="proto-me-ai-aspect-lock"
            onClick={toggleAspectLock}
            type="button"
          >
            <ProtoMeAspectLockIcon locked={isAspectLocked} />
          </button>
          <label className="proto-me-ai-size-field">
            <span>H</span>
            <input
      ...
      <section className="proto-me-ai-style-section">
        ...
        <div className="proto-me-ai-aspect-grid">
          {AI_IMAGE_ASPECT_PRESETS.map((preset) => (
            <button
              key={preset.id}
              aria-pressed={activePreset?.id === preset.id}
              className="proto-me-ai-aspect-preset"
              onClick={() =>
            ...
            >
              <span
                className="proto-me-ai-aspect-icon"
...
function ProtoMeAspectLockIcon({ locked }) {
  if (locked) {
    return (
      <svg
        aria-hidden="true"
        className="proto-me-ai-lock-icon"
        viewBox="0 0 20 20"
      >
...
function ProtoMeToolbarItem({ toolId }) {
  const editor = useEditor()
...
function ProtoMeAnnotationToolbarItem() {
  const editor = useEditor()
  const isSelected = useValue(
  ...
  return (
    <button
      ...
      className="tlui-button tlui-button__tool proto-me-annotation-toolbar-button"
    ...
    >
      {annotationToolIcon}
      <span className="proto-me-annotation-toolbar-label" draggable={false}>
...
function ProtoMeToolbarDivider() {
  return <div aria-orientation="vertical" className="proto-me-toolbar-divider" role="separator" />
}

function ProtoMeToolbar(props) {
  return (
    <DefaultToolbar {...props} maxItems={9}>
      <ProtoMeAnnotationToolbarItem />
      <ProtoMeToolbarDivider />
      <SelectToolbarItem />
      <HandToolbarItem />
      <ProtoMeToolbarItem toolId={AI_IMAGE_TOOL_ID} />
      <ProtoMeToolbarDivider />
      <AssetToolbarItem />
      <DrawToolbarItem />
      <EraserToolbarItem />
...
function getProtoMeSelection(editor) {
  const selectedShapeIds = editor.getSelectedShapeIds()
  return selectedShapeIds.map((id) => {
    ...
    return {
      ...
      isAiImageHolder: shape?.meta?.protoMeAiImageHolder === true,
...
function getProtoMeSelectionSnapshot(editor) {
  return {
    selectedShapes: getProtoMeSelection(editor)
  }
}

function getProtoMeViewState(editor) {
...
function restoreProtoMeViewState(editor, viewState) {
...
function writeProtoMeSelectionState(selectionSnapshot) {
...
export default function App() {
  ...
  const handleMount = useCallback((editor) => {
    window.__protoMeEditor = editor
    window.__protoMeSelection = () => getProtoMeSelection(editor)
    window.__protoMeViewState = () => getProtoMeViewState(editor)
    ...
    editor.timers.requestAnimationFrame(() => {
      restoreProtoMeViewState(editor, viewState)
    })

    async function syncSelectionState() {
      const selectionSnapshot = getProtoMeSelectionSnapshot(editor)
      writeProtoMeSelectionState(selectionSnapshot)
    ...
    if ('EventSource' in window) {
      ...
      canvasEvents.onerror = (error) => {
        console.warn('Proto-me canvas live refresh disconnected.', error)
      }
    }
    ...
    const unsubscribeAnnotationEditingToolLock = editor.store.listen(
      ({ changes }) => {
        for (const [previous, next] of Object.values(changes.updated)) {
          ...
          if (shape?.meta?.protoMeAnnotationArrow !== true) continue
    ...
    const unsubscribeAnnotationShapeSync = editor.store.listen(
      ({ changes }) => {
        ...
        for (const [_previous, next] of Object.values(changes.updated)) {
          ...
          if (next.meta?.protoMeAnnotationArrow !== true) continue
    ...
    return () => {
      ...
      remoteLoadController?.abort()
      canvasEvents?.close()
      if (window.__protoMeEditor === editor) {
        delete window.__protoMeEditor
        delete window.__protoMeSelection
        delete window.__protoMeViewState
      }
  ...
  if (snapshot === undefined || viewState === undefined) {
    return (
      <main className="proto-me-status" aria-live="polite">
        Loading canvas...
      </main>
    )
  }
  ...
  return (
    <main className="proto-me-canvas" aria-label="Proto-me infinite canvas">
      <SkippedRecordsNotice records={skippedRecords} />
      <Tldraw
        snapshot={snapshot ?? undefined}
        inferDarkMode
        onMount={handleMount}
        overrides={protoMeUiOverrides}
        components={protoMeComponents}
        shapeUtils={protoMeShapeUtils}
        tools={[ProtoMeAnnotationTool]}
      />
    </main>
  )
}
...
function SkippedRecordsNotice({ records }) {
  ...
  return (
    <aside className="proto-me-skipped-records" aria-live="polite">
