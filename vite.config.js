...
const projectDir = resolve(process.env.PROTO_ME_PROJECT_DIR ?? process.cwd())
const canvasSlug = sanitizeCanvasSlug(process.env.PROTO_ME_CANVAS_SLUG)
const defaultCanvasDir = canvasSlug ? join(projectDir, 'canvas', canvasSlug) : join(projectDir, 'canvas')
const canvasDir = resolve(process.env.PROTO_ME_CANVAS_DIR ?? defaultCanvasDir)
const canvasFile = join(canvasDir, 'proto-me-canvas.json')
const selectionFile = join(canvasDir, 'proto-me-selection.json')
const viewStateFile = join(canvasDir, 'proto-me-view-state.json')
const runtimeFile = join(canvasDir, 'proto-me-runtime.json')
...
const canvasFileName = 'proto-me-canvas.json'
...
async function writeJsonAtomic(filePath, payload) {
  ...
  const tempFile = `${filePath}.${process.pid}-${nonce}.tmp`
  await writeFile(tempFile, `${JSON.stringify(payload, null, 2)}\n`)
  await rename(tempFile, filePath)
}

async function writeRuntimeState(server) {
...
function canvasStoragePlugin() {
  return {
    name: 'proto-me-canvas-storage',
    configureServer(server) {
      server.httpServer?.once('listening', () => {
        writeRuntimeState(server).catch((error) => {
          server.config.logger.warn(`Failed to write Proto-me runtime state: ${error.message}`)
        })
      })
        ...
        try {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isSelectionState(selection)) {
              sendJson(res, 400, { error: 'Expected a Proto-me selection state.' })
              return
            }
        ...
        try {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isViewState(viewState)) {
              sendJson(res, 400, { error: 'Expected a Proto-me view state.' })
              return
            }
      ...
      server.middlewares.use('/api/canvas', async (req, res) => {
        try {
          if (req.method === 'GET') {
            ...
            sendJson(res, 200, { ...result, projectDir, canvasDir, canvasSlug })
            return
          }
          ...
          if (req.method === 'PUT') {
            ...
            sendJson(res, 200, { ok: true, ...result, projectDir, canvasDir, canvasSlug })
